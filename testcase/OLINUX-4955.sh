...
TESTCMD="cd $(basename ${TESTSRC})/bin && timeout 60 stdbuf -oL bash manhattan31_off.sh  2>&1 | tee ${JID}.out"

mkdir -p $TESTFOLDER/$JID/

sshpass -p ${Pass} ssh -o StrictHostKeyChecking=no root@${SOSIP} "export http_proxy=http://child-prc.intel.com:913 && export https_proxy=http://child-prc.intel.com:913 && swupd bundle-add libX11client"
