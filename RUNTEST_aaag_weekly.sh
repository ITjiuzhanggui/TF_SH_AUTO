...
# Get latest image
bash $BASEDIR/getbuild_aaag_weekly.sh | tee $BASEDIR/getbuild_aaag_weekly.out
latest=$(cat $BASEDIR/getbuild_aaag_weekly.out | tail -1 | awk -F': ' '{print $2}')
[ "$TESTVER" == "" ] && TESTVER=$latest

# Flash new image
bash $BASEDIR/fastboot_aaag.sh weekly/$TESTVER $USBNO
...
# Get Version
bash $BASEDIR/GETVER_aaag_weekly.sh $USBNO $ip $TESTVER
